...
public class VotesController : ControllerBase
{
    ...
    public VotesController(ApplicationDbContext context, IHubContext<VoteHub> hub)
    {
        _context = context;
        _hub = hub;
    }
    ...
    public async Task<ActionResult<VoteDto>> CreateVote(CreateVoteDto dto)
    {
        ...
        // Compute updated aggregate counts to broadcast
        var counts = await _context.Votes
            .Where(v => v.QuestionId == vote.QuestionId)
            .GroupBy(v => 1)
            .Select(g => new {
                QuestionId = vote.QuestionId,
                Side1Votes = g.Count(v => v.Choice == VoteChoice.Side1),
                Side2Votes = g.Count(v => v.Choice == VoteChoice.Side2),
                TotalVotes = g.Count()
            })
            .FirstOrDefaultAsync();
        ...
        if (counts != null)
        {
            await _hub.Clients.Group(VoteHub.GetGroupName(vote.QuestionId))
                .SendAsync("VoteUpdated", counts);
        }
    ...
    public async Task<ActionResult<VoteDto>> ChangeVote(CreateVoteDto dto)
    {
        ...
        var counts = await _context.Votes
            .Where(v => v.QuestionId == existingVote.QuestionId)
            .GroupBy(v => 1)
            .Select(g => new {
        ...
        if (counts != null)
        {
            await _hub.Clients.Group(VoteHub.GetGroupName(existingVote.QuestionId))
                .SendAsync("VoteUpdated", counts);
        }
