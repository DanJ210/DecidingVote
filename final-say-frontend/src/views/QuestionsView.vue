<template>
  <div class="max-w-5xl px-4 mx-auto">
    <h1 class="mb-6 text-3xl font-bold text-slate-800 dark:text-slate-100">All Questions</h1>
    
  <div v-if="loading" class="py-8 text-center text-gray-500 dark:text-slate-400">Loading questions...</div>
  <div v-else-if="error" class="p-4 mb-6 text-red-700 border border-red-200 rounded bg-red-50 dark:border-red-400/30 dark:bg-red-400/10 dark:text-red-300">{{ error }}</div>
    ...
    <div v-else>
      ...
      <div v-else class="space-y-6">
        <div v-for="question in questions" :key="question.id" class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-slate-700 dark:bg-slate-800">
          <div class="pb-4 mb-4 border-b border-gray-100 dark:border-slate-700">
          ...
          <div v-else-if="isAuthenticated && hasUserVoted(question.id) && canChangeVote(question.id)" class="mt-3">
            <div class="p-3 mb-3 text-sm border border-gray-200 rounded-md bg-gray-50 dark:border-slate-600 dark:bg-slate-700/50 dark:text-slate-200">
          ...
          <div v-else class="mt-4">
            <div class="flex flex-col gap-4">
              <div class="flex items-center gap-3">
                <span class="text-sm font-medium text-gray-700 min-w-16 dark:text-slate-300">{{ question.side1Text }}:</span>
                <span class="w-8 text-sm text-right text-gray-600 dark:text-slate-400">{{ question.side1Votes }}</span>
                <div class="flex-1 h-5 overflow-hidden bg-gray-200 rounded-full dark:bg-slate-700">
                  <div class="h-full transition-all bg-primary" :style="{ width: getSide1Percentage(question) + '%' }"></div>
                </div>
              </div>
              <div class="flex items-center gap-3">
                <span class="text-sm font-medium text-gray-700 min-w-16 dark:text-slate-300">{{ question.side2Text }}:</span>
                <span class="w-8 text-sm text-right text-gray-600 dark:text-slate-400">{{ question.side2Votes }}</span>
                <div class="flex-1 h-5 overflow-hidden bg-gray-200 rounded-full dark:bg-slate-700">
                  <div class="h-full transition-all bg-secondary" :style="{ width: getSide2Percentage(question) + '%' }"></div>
                </div>
              </div>
            </div>
            <p class="mt-3 text-xs tracking-wide text-gray-500 uppercase dark:text-slate-400">Total votes: {{ question.side1Votes + question.side2Votes }}</p>
          </div>
        </div>
      </div>
    </div>
...
import { useVoteHub } from '../composables/useVoteHub'
...
const { on: onHub, joinQuestion, disconnect } = useVoteHub()

onMounted(async () => {
  await fetchQuestions()
  if (isAuthenticated.value) await fetchUserVotes()

  onHub('VoteUpdated', (payload: { QuestionId: number; Side1Votes: number; Side2Votes: number; TotalVotes: number }) => {
    const q = questions.value.find(x => x.id === payload.QuestionId)
    if (q) {
      q.side1Votes = payload.Side1Votes
      q.side2Votes = payload.Side2Votes
    }
  })
})
...
watch(isAuthenticated, async (authed) => {
  if (authed) {
    // join all groups after login
    for (const q of questions.value) {
      try { await joinQuestion(q.id) } catch (e) { /* ignore */ }
    }
  } else {
    // disconnect hub when logged out
    await disconnect()
  }
})
